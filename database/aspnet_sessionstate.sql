...
-- Should install sessionstate support viav aspnet_regsql.exe instead
--   C:\WINDOWS\Microsoft.NET\Framework\v4.0.30319>aspnet_regsql.exe -S localhost -d %databasename% -ssadd -sstype c -U sa
...
IF DB_ID(N'%databasename%') IS NULL BEGIN
...
SET @jobname = N'%databasename%' + '_Job_DeleteExpiredSessions' 
...
IF (@ver >= 8)
    SET @cmd = N'
        ...
        AS
            CREATE TABLE [%databasename%].dbo.ASPStateTempSessions (
            ...
            CREATE NONCLUSTERED INDEX Index_Expires ON [%databasename%].dbo.ASPStateTempSessions(Expires)

            CREATE TABLE [%databasename%].dbo.ASPStateTempApplications (
                AppId               int             NOT NULL PRIMARY KEY,
                AppName             char(280)       NOT NULL,
            ) 

            CREATE NONCLUSTERED INDEX Index_AppName ON [%databasename%].dbo.ASPStateTempApplications(AppName)
...
SET @cmd = N'
    ...
    SELECT @appId = AppId
    FROM [%databasename%].dbo.ASPStateTempApplications
    ...
    IF @appId IS NULL BEGIN
        ...
        SELECT @appId = AppId
        FROM [%databasename%].dbo.ASPStateTempApplications WITH (TABLOCKX)
        WHERE AppName = @appName
        ...
        BEGIN
            ...
            INSERT [%databasename%].dbo.ASPStateTempApplications
            VALUES
            (@appId, @appName)
...
IF (@ver >= 8)
    SET @cmd = N'
        ...
        AS
            ...
            UPDATE [%databasename%].dbo.ASPStateTempSessions
            ...
            IF @length IS NOT NULL BEGIN
                READTEXT [%databasename%].dbo.ASPStateTempSessions.SessionItemLong @textptr 0 @length
            END
...
IF (@ver >= 8)
    SET @cmd = N'
        ...
        AS    
            ...
            INSERT [%databasename%].dbo.ASPStateTempSessions 
                (SessionId, 
                 SessionItemShort, 
                 Timeout, 
                 Expires, 
...
SET @cmd = N'
    CREATE PROCEDURE dbo.TempRemoveStateItem
        ...
        @lockCookie int
    AS
        DELETE [%databasename%].dbo.ASPStateTempSessions
        WHERE SessionId = @id AND LockCookie = @lockCookie
        RETURN 0'
EXEC(@cmd)    
GO
    ...
    SET @cmd = N'
        ...
        AS
            ...
            INSERT #tblExpiredSessions (SessionID)
                SELECT SessionID
                FROM [%databasename%].dbo.ASPStateTempSessions WITH (READUNCOMMITTED)
                WHERE Expires < @now
            ...
            BEGIN 
                ...
                WHILE @@FETCH_STATUS = 0 
                    BEGIN
                        DELETE FROM [%databasename%].dbo.ASPStateTempSessions WHERE SessionID = @SessionID AND Expires < @now
                        FETCH NEXT FROM ExpiredSessionCursor INTO @SessionID
                    END
...
BEGIN TRANSACTION            
    ...
    -- Add the job
    SET @nameT = N'%databasename%' + '_Job_DeleteExpiredSessions'
    ...
    -- Add the job steps
    SET @nameT = N'%databasename%' + '_JobStep_DeleteExpiredSessions'
    ...
    SET @nameT = N'%databasename%' + '_JobSchedule_DeleteExpiredSessions'
